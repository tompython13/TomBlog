...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(User.query.filter_by(email=form.email.data).first().password,
                                        form.password.data):
            login_user(user)
            return redirect("/")
        else:
            ...
            elif not check_password_hash(User.query.filter_by(email=form.email.data).first().password,
                                         form.password.data):
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash("The post title is duplicate with the existing post title. Please try again.")
...
def delete_post(post_id):
    ...
    comment_to_delete = Comment.query.all()
